...
const SERVER = "http://localhost:8080"; // đổi IP nếu cần

const webcamImg = document.getElementById("webcam");
const recordBtn = document.getElementById("recordBtn");
const secondsInput = document.getElementById("seconds");
function onWebcam() {
    webcamImg.src = `${SERVER}/onwebcam`;
}
function offWebcam() {
    fetch(`${SERVER}/offwebcam`);
    webcamImg.src = ""; // ngắt stream phía FE
}
async function recordWebcam() {
    const seconds = parseInt(secondsInput.value) || 5;

    // Gửi lệnh record
    await fetch(`${SERVER}/record?seconds=${seconds}`, {
        method: "POST"
    });

    alert(`Đang quay ${seconds} giây...`);

    // Đợi record xong rồi tải
    setTimeout(downloadLastVideo, (seconds + 1) * 1000);
}
async function downloadLastVideo() {
    const res = await fetch(`${SERVER}/last-record`);
    ...
    window.location.href =
        `${SERVER}/download?file=${encodeURIComponent(data.file)}`;
}
